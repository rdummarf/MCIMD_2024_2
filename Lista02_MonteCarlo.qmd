...
Iremos gerar uma amostra de tamanho 30 a partir de uma distribuição lognormal padrão (com média logarítmica $𝜇 = 0$ e desvio padrão logarítmico $\sigma = 1$ ).

O intervalo de confiança é calculado utilizando-se a fórmula:

$$
\left( \bar{y} - z_{\alpha/2} \cdot \frac{s_y}{\sqrt{n}}, \bar{y} + z_{\alpha/2} \cdot \frac{s_y}{\sqrt{n}} \right)
$$
...
print(paste("Proporção de ICs que contêm o verdadeiro valor de mi: ", mean(L_confid_interv < 0 & U_confid_interv > 0)))
