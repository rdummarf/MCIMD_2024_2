...
::: {.warning style="padding:0.1em; background-color:rgba(215,209,204,.3); color:#51247a"}
<span>
...
[^1]: If you want to render the R Notebook on your machine, i.e. knitting the document to html or a pdf, you need to make sure that you have R and RStudio installed and you also need to download the [**bibliography file**](https://slcladal.github.io/content/bibliography.bib) and store it in the same folder where you store the Rmd file.

<br>

::: {.warning style="padding:0.1em; background-color:#51247a; color:#f2f2f2"}
<span>
...
Click on this [![Binder](https://mybinder.org/badge_logo.svg)](https://binderhub.atap-binder.cloud.edu.au/v2/gh/SLCLADAL/interactive-notebooks-environment/main?urlpath=git-pull%3Frepo%3Dhttps%253A%252F%252Fgithub.com%252FSLCLADAL%252Finteractive-notebooks%26urlpath%3Dlab%252Ftree%252Finteractive-notebooks%252Fnotebooks%252Ftopictool.ipynb%26branch%3Dmain) badge to open an notebook-based tool <br>that allows you to upload your own text(s), perform topic modelling on them, and download the results.
...
## Human-in-the-loop Topic Modelling {.unnumbered}
...
4. Foram removidas frases que são comuns aos pedidos (ex: lei de acesso à informação)
5. Foram removidas algunas palavras de encerramento de e-mail (ex: por favor, atenciosamente), de marcação de imprensa (sic)
6. Foram removidas palavras com 3 ou mais letras repetidas, por se tratarem de ocultação de informações pessoais.
...
padroes <- list(
  "\\b[s]olicit\\w*\\b",  # Captura palavras que começam com "solicit"
  "\\b[o]bter\\b",        # Captura a palavra "obter"
  "\\b[r]equei\\w*\\b",     # Captura palavras que começam com "reque"
  "\\b[s]aber\\b",        # Captura a palavra "saber"
  "\\b[d]isponibilizar\\b", # Captura a palavra "disponibilizar"
  "\\b[r]eceb\\w*\\b",       # Captura a palavra "receber"
  "\\b[g]ostaria\\s+de\\s+\\w+\\b", # Captura "gostaria de" seguido de uma palavra
  "\\b[g]ostaria\\s+que\\b", # Captura a expressão "gostaria de"
  "\\b[n]ecessito\\s+de\\b", # Captura a expressão "necessito de"
  "\\b[n]ecessito\\s+da\\b", # Captura a expressão "necessito da"
  "\\b[n]ecessito\\s+do\\b" # Captura a expressão "necessito do"
)
...
topicmodels::LDA(ctxt, k = 6, control = list(seed = 1)) -> ddlda
